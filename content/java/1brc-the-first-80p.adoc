...
René Schwietzke
:jbake-date: 2025-12-20
:jbake-last_updated: 2025-12-20
:jbake-type: post
...
The https://www.morling.dev/blog/one-billion-row-challenge/[One Billion Row Challenge (1BRC)] started as a way to kill time during the 2023/2024 holidays, asking a simple question: How fast can Java crunch temperature values from a text file and calculate the min, mean, and max per weather station? The catch is the file has one billion rows, totaling roughly 13.8 GB.

The top result came from Thomas Wuerthinger, Quan Anh Mai, and Alfonso Peterssen at 1.5 s on 8 cores. That was insanely and unexpectedly fast. But when you take a closer look at the code, you will see that it is beyond your daily dose of Java. It is complex, single use-case oriented, skips all safety features, and most of the tunings applied cannot be easily reused. 

This article will show you how to get to an 80% of the result with simpler and more maintainable code. It will also focus on a single-thread approach to keep us focused. You can use most of the ideas in your own code easily, if you need such tuning. 

== The Original Challenge 
At the end of 2023, when everyone was busy with the holidays, https://www.morling.dev/[Gunnar Morling] got bored. So he came up with a fun little project to read one billion rows from a CSV file as fast as possible. And because it is only fun when the great Java community gets to it, he turned it into a competition. He wanted to see how fast people could make Java run.footnote:[https://www.morling.dev/blog/one-billion-row-challenge/[The One Billion Row Challenge]]

CAUTION: All runtime numbers have been measured by me on 8-core cloud machines at Hetzner (AMD CPUs) and Digital Ocean (Intel CPUs). This means your mileage might vary, and because machines and setups change, I might not be able to reproduce the exact numbers. You always have to measure the entire series to get a good picture of the runtimes.
...
* The file is encoded in UTF-8 and characters outside of the ASCII range.
...
=== The Outcome and Our Goal
The following results have been achieved. Once again, these are my measurements and not the official results. Also, as a reminder, we will avoid multi-threading for now to keep things as simple as possible. The baseline is straightforward Java code that does the job. This was the baseline suggested by Gunnar Morling.

.The Remeasured 1BRC Results
[cols="2,>1,>1,>1", options="header"]
|===
|Test |Cores|Time DO Intelfootnote:[Digital Ocean, Amsterdam, Dedicated Machine, Intel CPU-optimized, 16 core, 32 GB, Jan 2025
]|Time Hetzner AMDfootnote:[Hetzner, Finland, Dedicated Machine, AMD, 8 core, 32 GB, Jan 2025]
...
TIP: You might not want to brute-force apply the tuning here to your code. You have to have a problem first. Just because you can, doesn't mean you should. Clean and maintainable code always comes first.

=== The Daily Life Limits
When you look at the winning solution, you can see that it uses Unsafe, GraalVM and native builds, a ton of bit-magic, and a lot of other things that are not something you would use in your own code. Impressive, interesting, but with limited daily life reuse. 

If you want to dive into the final solution, try to understand the https://questdb.com/blog/1brc-merykittys-magic-swar/[branch-less parsing] because it is the key driver for the performance compared to other solutions as well as our's today.

== Our 80% Goal
To save 80% of the runtime and ensure a proper result all the time, I wrote a small framework for executing the 1BRC task and verifying the result with SHA-512. By comparing the checksum against the original baseline solution's output, I ensure nothing has been broken.

By the way, I really broke the solution several times while tuning. The worst bug was only happening when changing the input data set. That took ages to find.

*Just to repeat it quickly*: We want to save 80% of the runtime and only go with one thread for simplicity.

You can find the code at https://github.com/rschwietzke/1brc-the-first-80-meters. I will either link to the implementation or just mention the class by name.

=== The Measurement Framework
Just to detour quickly. The original 1BRC uses single Java files and always runs them several times. To make measurements easier and also allow for better learning in regards to Java JIT compile, I wrote a https://github.com/rschwietzke/1brc-the-first-80-meters/blob/main/src/main/java/dev/morling/onebrc/Baseline.java[small framework] for executing each solution including measurement, warming, and checksumming, not to forget batch execution.

== Step 0: The Baseline
The baseline is the original solution by Gunnar Morling. 
...
As you can see, we have a plain stream implementation that uses features Java provides, such as collecting and grouping. I leave the interpretation to you or just read about that in all of the other 1BRC articles out there. AI is your friend here.

== Step 1: Get to Know the Code
The above code runs for a moment, but we don't know yet where it might spend most of its time. We don't want to guess what to optimize; we want to see. For that, we look into our toolbox and find the https://github.com/async-profiler/async-profiler[AsyncProfiler]. The most wonderful open-source profiler for Java. 
...
CAUTION: This code is not 100% correct due to the inherent imprecision of floating-point calculations on CPUs, but it works well enough in our case due to the limitation to one decimal place.
...
We gained a lot on the Intel CPU. For AMD, we got it 10 seconds faster.
...
So, some gain for AMDs, but not much for Intel.

== Step 9: Turn the Map Into a Set
Our current map has a disadvantage. It needs two slots and it needs a wrapper for the values. Why not combine that into a single entry? And so we get a set.
...
Ok, we gained about 2%. Not much, but we are getting there.
...
Our `readLine` contains a method call to `read` and that is 2.2% of the runtime. The rest is used for our data processing. Yes, it could have been extracted nicer into its own method. Next time!

== The Unexpected Bonus
When we run our baseline code with EpsilonGC, we get about 3.2 GB of heap usage for 10 million lines. That sums up to a staggering 320 GB for 1 billion rows. That is memory churn at the end of the day. Request memory, clean it, use it, investigate if still in use, add it to the free list or move the content around when still in use. By the way, We are not really allocating and deallocating against the OS here, because the JVM does its own memory segment management (ask for a chunk of memory and own that as long as possible),

NOTE: EpsilonGC is a GC that does not do any GC at all. It just reserves or assigns memory, but never frees it. Hence, a program might crash after a while if it needs more memory than what the heap can hold. Perfect for tuning and investigations because all background GC processes are offline.
...
== Behind the Scenes
For the fans of hard-core performance and low-level facts, let's dive into the CPU and memory details. These statistics are provided by the https://perfwiki.github.io/main/tutorial/#counting-with-perf-stat[`perf`] tool, courtesy of the Linux kernel.
...
|*Branch-misses* | 2,479 M (0.47%) | 1,540 M (2.76%) | 149,0 M (0.95%)
...
* The number of branches (ifs and loops) dropped by a factor of five from baseline to our BRC45.
...
* The reduction in the number of branches is insane and mostly comes from branchless number parsing.
...
.Different Machines Types and Their Performance
...
== Summary
The One Billion Row Challenge is a fantastic way to explore Java performance. By applying a systematic tuning approach, starting from a clean baseline and incrementally optimizing everything from stream usage to I/O and custom collection implementations, we achieved an 80% reduction in runtime using only standard Java features. While `Unsafe` is not a standard feature in my view, and extreme bit magic is fully legal but makes the code difficult to understand for most people, those techniques fall under: only when truly needed, only by experts, and only with a ton of documentation.
